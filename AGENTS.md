...
| 运营后台 | Vite + TypeScript（原生 DOM SPA），线上由 Cloudflare Access Email OTP 保护 |
...
- 运营 Web：`https://whatsapp.novelvela.com`（Cloudflare Pages + Access Email OTP，Allow `magicxiaomin@gmail.com`；`/api/*` 同源代理到 Worker admin API；当前 Pages custom domain 等待 DNS CNAME 验证）。过渡期 `dm-broadcast-web.pages.dev` 与 `*.dm-broadcast-web.pages.dev` 也被同一个 Access app 保护，避免无 demo 门的公开窗口。
- Worker：`dm-broadcast-api` → `https://dm-broadcast-api.magicxiaomin.workers.dev`（设备 API host，不加交互式 Access）
...
```
运营后台 Web (apps/web, whatsapp.novelvela.com)
   │ POST /v1/campaigns · GET /v1/dashboard · POST /v1/tasks/requeue
   │ 线上经 /api/* 同源代理，并由 Cloudflare Access 注入 Cf-Access-Jwt-Assertion
   ▼
...
`users` · `devices` · `contacts` · `device_contacts` · `campaigns` · `tasks` · `im_events` · `ledger_entries`

要点：
- `users`：运营后台维护的 creator/user 账号；无登录/鉴权语义。`devices.user_id` nullable，一设备最多归属一个 user。
- `contacts`：历史全局联系人目录，`wa_jid` 仍全局唯一，保留给现有兼容路径；不表达设备/用户归属。
- `device_contacts`：联系人归属真相源，`PRIMARY KEY(device_id, wa_jid)`，允许同一 `wa_jid` 同时属于多台设备；已有全局 `contacts` 不回填归属。
...
|---|---|---|
| GET | /health | 健康检查（公开） |
| GET | /v1/dashboard | 后台汇总（设备/任务/事件/ledger，需 Access JWT 或 ADMIN_TOKEN） |
| GET/POST | /v1/devices · /v1/devices/register | 设备列表（Access JWT 或 ADMIN_TOKEN） / 注册（DEVICE_TOKEN，含 safety 上报） |
| GET/POST | /v1/users · /v1/devices/assign | 运营维护 user 列表/创建（含已入账与待确认聚合） / 设备归属绑定（Access JWT 或 ADMIN_TOKEN） |
| GET/POST | /v1/contacts · /v1/contacts/sync | 联系人列表（Access JWT 或 ADMIN_TOKEN；支持 `deviceId`/`userId` 归属过滤） / 同步（DEVICE_TOKEN；必须显式 `deviceId`） |
| GET/POST | /v1/campaigns | 广播列表 / 创建（展开 tasks，需 Access JWT 或 ADMIN_TOKEN；创建必须显式传 deviceId） |
| GET | /v1/tasks · /v1/tasks/pull | 任务列表（Access JWT 或 ADMIN_TOKEN） / 原子认领（DEVICE_TOKEN；必须显式传 deviceId；严格 `device_id = ?`；safety gate + stale 释放） |
| POST | /v1/tasks/requeue | failed/claimed → pending（需 Access JWT 或 ADMIN_TOKEN） |
| POST | /v1/events | message_sent/failed/read/message_ack（需 DEVICE_TOKEN；ack 按 server_msg_id 归因加分） |
| GET | /v1/ledger | 积分流水（需 Access JWT 或 ADMIN_TOKEN） |
| POST | /v1/admin/cleanup-test-data | 清测试数据（需 Access JWT 或 ADMIN_TOKEN） |

鉴权现状：B18 已落地。`/health` 公开；运营人侧走 `https://whatsapp.novelvela.com`，由 Cloudflare Access Email OTP 保护，Worker 对运营路由双接受：先验 `Authorization: Bearer <ADMIN_TOKEN>`（脚本/本地/CI 兜底），再验 `Cf-Access-Jwt-Assertion`（`CF_ACCESS_TEAM_DOMAIN` + `CF_ACCESS_AUD`，jose 验签/AUD/iss）。设备路由只接受共享 `DEVICE_TOKEN`，继续走 `https://dm-broadcast-api.magicxiaomin.workers.dev`，不加交互式 Access。`DEVICE_TOKEN` 仍是 MVP 共享设备钥匙，不是 per-device token；spec 设想的 `/auth/bind` 弱证明与每设备 token 仍留在 B2-full backlog。
...
| B17a | 联系人归属设备/用户后端（device_contacts + 归属查询） | 已落地 |
| B18 | 后台登录 via Cloudflare Access Email OTP | 已落地 |

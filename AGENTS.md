...
- 运营 Web：`https://whatsapp.dramavela.com`（Cloudflare Pages + Access Email OTP，Allow `magicxiaomin@gmail.com`；`/api/*` 同源代理到 Worker admin API；Pages custom domain 已 active）。过渡期 `dm-broadcast-web.pages.dev` 与 `*.dm-broadcast-web.pages.dev` 也被同一个 Access app 保护，避免无 demo 门的公开窗口。
...
```
运营后台 Web (apps/web, whatsapp.dramavela.com)
